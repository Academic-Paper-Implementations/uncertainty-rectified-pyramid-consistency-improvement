...
parser.add_argument('--sharpen_T', type=float, default=0.5,
                    help='temperature for target sharpening (T < 1 => sharper probabilities)')
parser.add_argument('--entropy_weight', type=float, default=0.01,
                    help='weight for unlabeled entropy minimization loss')
...
parser.add_argument('--attention_type', type=str, default='lightweight',
...
def train(args, snapshot_path):
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            # Target Sharpening: make unlabeled targets more confident
            T = args.sharpen_T
            unlabeled_preds = preds[args.labeled_bs:]
            preds_sharpened = unlabeled_preds ** (1.0 / T)
            preds_sharpened = preds_sharpened / (preds_sharpened.sum(dim=1, keepdim=True) + 1e-8)
            preds = torch.cat(
                [preds[:args.labeled_bs], preds_sharpened],
                dim=0
            ).detach()
            # ========================================================================
            ...
            # Unlabeled entropy minimization on main branch
            unlabeled_outputs = outputs_soft[args.labeled_bs:]
            entropy_loss = -torch.mean(
                torch.sum(unlabeled_outputs * torch.log(unlabeled_outputs + 1e-8), dim=1)
            )

            loss = supervised_loss + consistency_weight * consistency_loss + args.entropy_weight * entropy_loss
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            ...
            writer.add_scalar('info/entropy_loss', entropy_loss, iter_num)
