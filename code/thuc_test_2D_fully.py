...
parser.add_argument('--labeled_num', type=int, default=3,
                    help='labeled data')
# Output path for model (should match training output_path)
parser.add_argument('--output_path', type=str,
                    default='/kaggle/working/model',
                    help='path where model weights are saved (default for Kaggle)')
...
def Inference(FLAGS):
    ...
    snapshot_path = os.path.join(FLAGS.output_path, "{}_{}_labeled".format(
        FLAGS.exp, FLAGS.labeled_num), FLAGS.model)
    test_save_path = os.path.join(FLAGS.output_path, "{}_{}_labeled".format(
        FLAGS.exp, FLAGS.labeled_num), "{}_predictions/".format(FLAGS.model))
